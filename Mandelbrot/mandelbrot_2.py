...
def mandelbrot_set(xmin, xmax, ymin, ymax, width, height, maxiter):
    ...
    #list that stores all complext values
    complex_list = [] #list that stores all complex values
    vmandel_res = [] #stores return values of complex number that underwent function mandelbrot()
    ...
    for i in range(width):
        ...
        for j in range(height):
            complex_list.append(cols[i] + 1j*rows[j])
            #arr_img[i,j] = mandelbrot(cols[i] + 1j*rows[j], maxiter)

    vmandelbrot = np.vectorize(mandelbrot)
    print("np.vectorize DONE")
    vmandel_res = vmandelbrot(complex_list, maxiter)
    print("vmandelbrot DONE")
    arr_img = np.reshape(vmandel_res, (width, height))
...
#You can "hard code" edit your values here
#if you don't want to use mandelbrot.py
